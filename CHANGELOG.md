...
- **Terminal text overflow / line wrapping in Claude Code**: replaced FitAddon's `proposeDimensions()` with Ghostty-style manual grid sizing — measures actual cell dimensions from xterm's render service, actual container size via `getBoundingClientRect`, and real scrollbar width from the viewport element (0 on macOS overlay scrollbars vs FitAddon's hardcoded 15 px fallback). Applies a 1 px safety margin before floor-dividing to absorb IEEE 754 edge cases, `clientWidth` rounding, and sub-pixel font rendering. This fixes border lines in Claude Code wrapping by 1 character.
- **Focus mode blank screen on session switch**: focus mode now keeps ALL session terminals mounted (stacked with `visibility: hidden`), only showing the active panel. Previously, switching sessions unmounted the old terminal (destroying its xterm buffer) and mounted a fresh empty one — all output for inactive sessions was silently dropped because no writer was registered. Switching is now instant with full terminal state preserved.
- **React hooks ordering crash in TerminalGrid**: moved `useState` and `useCallback` hooks above the early return so they are called unconditionally on every render, fixing the "Rendered more hooks than during the previous render" error.
- **PTY resize not sent on container resize**: `syncTerminalSize()` now sends the new cols/rows to the PTY via `resizeSession` on every fit — previously the ResizeObserver callback only called `fit()` without forwarding the dimensions to the backend.
...
- **xterm CSS reset corruption**: restored `content-box` sizing inside the `.xterm` subtree so the app-wide `border-box` reset and Tailwind preflight do not break xterm.js cell measurement. This fixes scattered characters, duplicated rows, and broken layout in full-screen TUIs like Claude Code.
- **UTF-8 glyph corruption at chunk boundaries**: PTY output is now decoded as a stream instead of per-read `from_utf8_lossy`, so box-drawing characters and other multibyte glyphs no longer turn into replacement characters when a `read()` splits them across chunks.
